...
public class PacketHandlerMail implements IPacketHandler {
	...
	private void onPOBoxInfo(PacketPOBoxInfo packet) {
		PluginMail.proxy.setPOBoxInfo(Proxies.common.getRenderWorld(), Proxies.common.getClientInstance().thePlayer.getGameProfile().getId().toString().replace("-", ""), packet.poboxInfo);
	}
	...
	private void onPOBoxInfoRequest(EntityPlayer player) {
		POBox pobox = PostRegistry.getPOBox(player.worldObj, player.getGameProfile().getId().toString().replace("-", ""));
		if (pobox == null)
			return;
