	...
	public static class FlutterlyzerInventory extends ItemInventory implements IErrorSource, IHintSource {
		...
		private void tryAnalyze() {
			...
			if (!butterfly.isAnalyzed()) {
				...
				butterfly.analyze();
				if (player != null) {
					PluginLepidopterology.butterflyInterface.getBreedingTracker(player.worldObj, player.getGameProfile().getId().toString().replace("-", "")).registerSpecies(butterfly.getGenome().getPrimary());
					PluginLepidopterology.butterflyInterface.getBreedingTracker(player.worldObj, player.getGameProfile().getId().toString().replace("-", "")).registerSpecies(butterfly.getGenome().getSecondary());
				}
