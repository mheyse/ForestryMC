	...
	public static class TreealyzerInventory extends ItemInventory implements IErrorSource, IHintSource {
		...
		private void tryAnalyze() {
			...
			if (!tree.isAnalyzed()) {
				...
				tree.analyze();
				if (player != null) {
					PluginArboriculture.treeInterface.getBreedingTracker(player.worldObj, player.getGameProfile().getId().toString().replace("-", "")).registerSpecies(tree.getGenome().getPrimary());
					PluginArboriculture.treeInterface.getBreedingTracker(player.worldObj, player.getGameProfile().getId().toString().replace("-", "")).registerSpecies(tree.getGenome().getSecondary());
				}
