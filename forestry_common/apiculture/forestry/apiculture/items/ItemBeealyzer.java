	...
	public static class BeealyzerInventory extends ItemInventory implements IErrorSource, IHintSource {
		...
		private void tryAnalyze() {
			...
			if (!bee.isAnalyzed()) {
				...
				bee.analyze();
				if(player != null) {
					PluginApiculture.beeInterface.getBreedingTracker(player.worldObj, player.getGameProfile().getId().toString().replace("-", "")).registerSpecies(bee.getGenome().getPrimary());
					PluginApiculture.beeInterface.getBreedingTracker(player.worldObj, player.getGameProfile().getId().toString().replace("-", "")).registerSpecies(bee.getGenome().getSecondary());
				}
